...
However, in reality spatial effects can play a significant role for population survival forcing some species develop competitive mechanisms such as cell adhesion to surfaces in the areas rich with nutrients~\cite{htuson_bacteriasurface_2013}.
...
It has to be noted that the resource pool is normalized by its initial value, i. e., $R^* = R/R_0$.
Moreover, the species $A$ is able to release a certain antimicrobial component $I$ that shows inhibitory effect on species 2 with any harmful effect on itself (amensalism).
...
\end{equation}
%
Using such matrix representation of the bacterial system, we can rewrite introduced above system in more compact form.
...
\end{equation}
%
With introduction of the limiting resource the system dynamics presented by the \acp{ode}~\ref{eq:ode_3pool_resource} will be rewritten in the following non-linear system:
\begin{eqnarray}

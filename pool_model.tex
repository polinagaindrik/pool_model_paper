...
According to the growth curves (\cmt{some references on bacteria growth curves??}) \cite{buchananLifePhasesBacterial1918, rolfeLagPhaseDistinct2012} quite often at the beginning no increase in bacterial count is present and so-called lag-phase is observed.
This phenomenon appears due to adaptation of the microbial community, for example, to the new environmental conditions.
\cmt{\cite{rolfeLagPhaseDistinct2012}: About Lag-phase machanisms.}
...
Based on experimental observations, it was shown that the bacterial growth is strongly influenced by the limiting resource. \cmt{ref review}
In the system where all needed nutritients are in excess except for one the total growth is linear to the conentration of this liminting nutritient. \cmt{ref Monod}
\cite{monodGrowthBacterialCultures1949}
\cmt{Ref about Quorum sensing. Signalling mechanism that controll gene expression (bacteria growth within species) in response to environment. \cite{ngBacterialQuorumsensingNetwork2009}}
...
These survival mechanisms mostly aim to secure as much available nutrition which lead to competitive strategies or, if it is advantageous, cooperative ones. \cite{hibbingBacterialCompetitionSurviving2010, stubbendieckBacterialCommunitiesInteractions2016} \cmt{Both good reviews on bacterial interaction}
...
\cmt{\cite{h.tusonBacteriaSurfaceInteractions2013}: Attachment of bacteria to surfaces. (One of the mechanisms to stick to the areas with nutritients)}
...
\cmt{Ref Tilman 1977 \cite{tilmanResourceCompetitionPlankton1977}, Ref Smith 2002 \cite{smithEffectsResourceSupplies2002}}
...
which has the structure of a generalised Lotka-Volterra system. \cmt{Add reference about Lotka-Volterra. Compare with different states LV.}
...
\cmt{\cite{lenskiCoexistenceTwoCompetitors1986}: Modelling of the 2 sp. system with 1resource and 1 inhibitor.}
...
Quite often it happens between the species that are close relatives or clonal populations and such process is called kin selection. \cmt{Ref 25 from review}
Sometimes neighbours, so-called 'social cheaters', can benefit from a certain behaviour without expressing it by itself.  \cmt{Ref 29 from review}

\cmt{\cite{rochaBaseCompositionBias2002}: Symbiosis host/bacteria. bacterial genomes suffering reductive evolution due to this. Maybe it's connected with 'social cheaters'?}

...
\begin{equation}
    \dot{\gamma} = \Gamma f\left(\dot{T}\right)-\delta \gamma.
\end{equation}
The dimensionless function $f$, with $f(0)=0$ and $f: \mathbb{R}\to\mathbb{R}_{\ge 0}$, evaluates the effect of a temperature change, i.e., a change from $T=2$ to $T=12$ could have a different effect as a change from $T=14$ to $T=4$. $\Gamma$ is a scaling factor and $\delta$ is the relaxation time for the temperature disturbance.
Integration of this equation yields (with $\gamma(0)=0$):
\begin{equation}
    \gamma(t) = \Gamma \int_0^t f\left(\dot{T}(t^{\prime})\right)e^{-\delta (t-t^{\prime})}dt^{\prime}.
\end{equation}
...
\begin{equation}
    \gamma(t) = \Gamma\alpha\left [T(t)-T(0)e^{-\delta t}-\delta
        \int\limits_0^t T(t^{\prime})e^{-\delta (t-t^{\prime})}dt^{\prime}\right ],
\end{equation}
for $\dot{T}\ge 0, \,\forall t$ and
\begin{equation}
    \gamma(t) = \Gamma(1-\alpha)\left [T(0)e^{-\delta t}-T(t)+\delta
        \int\limits_0^t T(t^{\prime})e^{-\delta (t-t^{\prime})}dt^{\prime}\right ].
\end{equation}
...
\begin{eqnarray}
    \gamma(t) &=& \Gamma\sum_{i=1}^n f\left(\Delta T_i \right)\theta(t-t_i)e^{-\delta(t-t_i)}.
...
\begin{figure}[t]
    ...
    \caption{
        ...
        The parameters of the back-flow rate are $\Gamma=1^\circ C / s$ and $\delta=5~\mathrm{s}^{-1}$.

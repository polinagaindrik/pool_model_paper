...
\begin{align}\begin{split}
    ...
    \dot{G} &= \lambda L + \alpha G - \omega' G\\
...
\begin{align}
    \begin{split}
        \dot{L} &= -(\omega + \lambda) L\\
        \dot{G} &= \lambda L + \alpha \tilde{R} G-\omega' G\\
        \dot{D} &= \omega  L + \omega' G\\
...
\begin{align}
    \begin{split}
        \dot{L} &= -(\omega + \lambda) L\\
        \dot{G} &= \lambda L + \alpha G\left(1-\frac{L+G+D}{N_t}\right)-\omega' G\\
        \dot{D} &= \omega  L + \omega' G.
    \end{split}
...
\begin{figure}
    ...
    \caption{
        ...
        All curves were calculated using the model (\ref{eq:ode_3pool_resource2}) with parameters $\alpha=2.5~\mathrm{s}^{-1}$, $\omega'=0.1~\mathrm{s}^{-1}$, $N_t = 10^4$.
...
\begin{align}
    ...
    1  &= 2\lambda e^{\alpha t_L}\frac{1-e^{-(\omega+\lambda+\alpha)t_L}}{\omega+\lambda+\alpha}.
\end{align}
For $\alpha > \omega+\lambda$  we can approximate $t_L$ by:
\begin{align}
    t_L &\approx -\frac{1}{\alpha}\ln\left(\frac{2\lambda}{\omega+\lambda+\alpha}\right).
\end{align}
...
\begin{figure}[t]
    ...
    \caption{
        ...
        The other model parameters are $\alpha=2.5~\mathrm{s}^{-1}$, $\omega'=0.5~\mathrm{s}^{-1}$, $\beta=5~\mathrm{s}^{-1}$, $\lambda=10^{-2}~\mathrm{s}^{-1}$, $\omega = 0~\mathrm{s}^{-1}$ for each curve.
    }
    \label{fig:TempJump}
    \end{center}
\end{figure}
...
\begin{figure}
    ...
    \caption{
        ...
        The red curve describes the growth of the bacteria $A$ ($N_A = L_A+G_A$) with model parameters $\lambda_A=10^{-2}~\mathrm{s}^{-1}$, $\alpha_A=3.0~\mathrm{s}^{-1}$.
        The blue curve describes the growth of the bacteria $B$ ($N_B = L_B+G_B$) with model parameters $\lambda_B=5\cdot 10^{-2}~\mathrm{s}^{-1}$, $\alpha_B=2.5~\mathrm{s}^{-1}$.
...
\begin{align}
    \begin{split}
        \dot{L}_A &= - \lambda_A R L_A\\
        \dot{L}_B &= - \lambda_B R L_B \\
        \dot{G}_A &= \lambda_A R L_A + \alpha_A R G_A - \frac{\omega}{\kappa N_t} T G_A\\
        \dot{G}_B &= \lambda_B R L_B + \alpha_B R G_B\\ 
        \dot{R} &= -\frac{\alpha_A}{N_t} R G_A-\frac{\alpha_B}{N_t} R G_B\\
...
\begin{align}
    \begin{split}
        ...
        \dot{G}_A &= \lambda_A R L_A + \alpha_A R G_A\\
        \dot{L}_B &= -\lambda_B R L_B\\
        \dot{G}_B &= \lambda_B R L_B + \frac{\alpha_B}{1 + \omega_I I} R G_B.
    \end{split}
...
\begin{align}
    \begin{split}
        \dot{R} &= -\frac{\alpha_A}{N_t} R G_A-\frac{\alpha_B}{(1 + \omega_I I) N_t} R G_B \\
        \dot{I} &= \kappa G_A.
    \end{split}
...
\begin{align}
    \dot{L}_A &= -\lambda_A R L_A\\
    \dot{L}_B &= -\lambda_B R L_B\\
    \dot{G}_A &= \alpha_A R G_A + \lambda_A R L_A\\
    \dot{G}_B &= \frac{\alpha_B}{1 + \omega_I I} R G_B + \lambda_B R L_B\\
    \dot{R} &= -\frac{\alpha_A}{N_t} R G_A-\frac{\alpha_B}{(1 + \omega_I I) N_t} R G_B\\
    \dot{I} &= \kappa G_A
\end{align}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{@{}l *2{>{\centering\arraybackslash}X} l *2{>{\centering\arraybackslash}X}@{}}
        ...
        Inhibition                  & $\omega_I$    & $1.0\cdot 10^{-7}$~\unit{\per\micro\metre\cubed}

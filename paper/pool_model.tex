...
\begin{figure}
    ...
    \caption{
        ...
        All curves were calculated using the model (\ref{eq:ode_3pool_resource2}) with parameters $\mu=2.5~\mathrm{h}^{-1}$, $\omega'=0.1~\mathrm{h}^{-1}$, $N_t = 10^4$.
        The blue curve includes cell-death of the bacteria in the lag phase $\omega = 0.15~\mathrm{h}^{-1}$ and thus exhibits a small dip before its exponential growth.
        Other curves show the case of no bacteria dying $\omega=0~\mathrm{h}^{-1}$ and represent different lengths of lag phases: short $\lambda=10^{-2}~\mathrm{h}^{-1}$ (dark green), intermediate $\lambda=10^{-3}~\mathrm{h}^{-1}$ (light green) and long one $\lambda=10^{-4}~\mathrm{h}^{-1}$ (yellow). Note that the steady state for $N(t)$ and $D(t)$ is for this model $\lim_{t\to\infty} N(t)=0$ and $\lim_{t\to\infty} D(t)=N_t$, because of the depletion of the resource pool.
    }

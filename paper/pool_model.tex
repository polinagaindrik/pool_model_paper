...
\begin{figure}[t]
    ...
    \caption{
        ...
        The curves represent no temperature shift (green), one temperature jump $\Delta T = 10^\circ C$ at time $t=4~\mathrm{h}$ (orange)
        ...
        The parameters of the back-flow rate are $\zeta=1 s/K $ and $\delta=5~\mathrm{h}^{-1}$.
        The other model parameters are $\mu=2.5~\mathrm{h}^{-1}$, $\omega'=0.5~\mathrm{h}^{-1}$, $\beta=5~\mathrm{h}^{-1}$, $\lambda=10^{-2}~\mathrm{h}^{-1}$, $\omega = 0~\mathrm{h}^{-1}$ for each curve.
    }
    \label{fig:TempJump}
    \end{center}
\end{figure}

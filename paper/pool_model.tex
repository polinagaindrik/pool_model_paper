...
Then $[\mu_0]=\unit{V/s}$, $[N_t]=\unit{1/V}$, and $[\mu]= \unit{1/s}$.
...
\begin{figure}
    ...
    \caption{
        ...
        All curves were calculated using the model (\ref{eq:ode_3pool_resource2}) with parameters $\mu=2.5~\mathrm{s}^{-1}$, $\omega'=0.1~\mathrm{s}^{-1}$, $N_t = 10^4$.
        The blue curve includes cell-death of the bacteria in the lag phase $\omega = 0.15~\mathrm{s}^{-1}$ and thus exhibits a small dip before its exponential growth.
        Other curves show the case of no bacteria dying $\omega=0~\mathrm{s}^{-1}$ and represent different lengths of lag phases: short $\lambda=10^{-2}~\mathrm{s}^{-1}$ (dark green), intermediate $\lambda=10^{-3}~\mathrm{s}^{-1}$ (light green) and long one $\lambda=10^{-4}~\mathrm{s}^{-1}$ (yellow). Note that the steady state for $N(t)$ and $D(t)$ is for this model $\lim_{t\to\infty} N(t)=0$ and $\lim_{t\to\infty} D(t)=N_t$, because of the depletion of the resource pool.
    }
...
\end{equation}
The dimensionless function $f$, with $f(0)=0$ and $f: \mathbb{R}\to\mathbb{R}_{\ge 0}$, evaluates the effect of a temperature change, i.e., a change from $T=2$ to $T=12$ could have a different effect as a change from $T=14$ to $T=4$. $\zeta$ is a scaling factor, $\sigma$ is a kind of stiffness telling how strongly the bacteria react to temperature stress, and $\delta^{-1}$ is the relaxation time for the temperature disturbance.
...
\end{align}
$\alpha\in [0,1]$. For $\alpha=0.5$ it follows $f(x)=\frac{1}{2}|x|$. Using this choice for $f$ we find after some manipulation:
\begin{equation}
...
\begin{align}
    \begin{split}
        \dot{L} &=\gamma(t) G - (\omega + \lambda) L\\
        \dot{G} &= \lambda L -\gamma(t) G + \mu G\left(1-\frac{L+G+D}{N_t}\right)\\
        \dot{D} &= \omega  L
    \end{split}
...
\begin{figure}[t]
    ...
    \caption{
        ...
        The curves represent no temperature shift (green), one temperature jump $\Delta T = 10^\circ C$ at time $t=4~\mathrm{s}$ (orange)
        and the series of different temperature shifts of value $10^\circ C$, $5^\circ C$ and $15^\circ C$ at the time points $1, 3$ and $7$, respectively (rose).
        The parameters of the back-flow rate are $\zeta=1 s/K $ and $\delta=5~\mathrm{s}^{-1}$.
        The other model parameters are $\mu=2.5~\mathrm{s}^{-1}$, $\omega'=0.5~\mathrm{s}^{-1}$, $\beta=5~\mathrm{s}^{-1}$, $\lambda=10^{-2}~\mathrm{s}^{-1}$, $\omega = 0~\mathrm{s}^{-1}$ for each curve.
    }
    \label{fig:TempJump}
    \end{center}
\end{figure}
...
\begin{figure}
    ...
    \caption{
        ...
        The red curve describes the growth of the bacteria $A$ ($N_A = L_A+G_A$) with model parameters $\lambda_A=10^{-2}~\mathrm{s}^{-1}$, $\mu_A=3.0~\mathrm{s}^{-1}$.
        The blue curve describes the growth of the bacteria $B$ ($N_B = L_B+G_B$) with model parameters $\lambda_B=5\cdot 10^{-2}~\mathrm{s}^{-1}$, $\mu_B=2.5~\mathrm{s}^{-1}$.
...
\begin{figure}
    ...
    \caption{
        ...
        with production rate $\kappa=0.2~\mathrm{s}^{-1}$.
        The toxin $T_B$ causes the cell death of bacteria $A$ with rate constant $\omega = 0.2~\mathrm{s}^{-1}$.
        The red curve describes the growth of the bacteria $A$ ($N_A = L_A+G_A$) with model parameters $\lambda_A=10^{-2}~\mathrm{s}^{-1}$, $\mu_A=3.0~\mathrm{s}^{-1}$.
        The blue curve describes the growth of the bacteria $B$ ($N_B = L_B+G_B$) with model parameters $\lambda_B=5\cdot 10^{-2}~\mathrm{s}^{-1}$, $\mu_B=2.5~\mathrm{s}^{-1}$.
        The gray line denotes the dynamic of the resource pool scaled with its capacity $N_t=10^4$.
    }

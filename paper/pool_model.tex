...
\begin{figure}
    ...
    \caption{
        ...
        All curves were calculated using the model (\ref{eq:ode_3pool_resource2}) with parameters $\alpha=2.5~\mathrm{s}^{-1}$, $\mu'=0.1~\mathrm{s}^{-1}$, $N_t = 10^4$.
        The blue curve includes cell-death of the bacteria in the lag phase $\mu = 0.15~\mathrm{s}^{-1}$ and thus exhibits a small dip before its exponential growth.
        Other curves show the case of no bacteria dying $\mu=0~\mathrm{s}^{-1}$ and represent different lengths of lag phases: short $\lambda=10^{-2}~\mathrm{s}^{-1}$ (dark green), intermediate $\lambda=10^{-3}~\mathrm{s}^{-1}$ (light green) and long one $\lambda=10^{-4}~\mathrm{s}^{-1}$ (yellow). Note that the steady state for $N(t)$ and $D(t)$ is for this model $\lim_{t\to\infty} N(t)=0$ and $\lim_{t\to\infty} D(t)=N_t$, because of the depletion of the resource pool.
    }
...
\begin{figure}[t]
    ...
    \caption{
        ...
        The other model parameters are $\alpha=2.5~\mathrm{s}^{-1}$, $\mu'=0.5~\mathrm{s}^{-1}$, $\beta=5~\mathrm{s}^{-1}$, $\lambda=10^{-2}~\mathrm{s}^{-1}$, $\mu = 0~\mathrm{s}^{-1}$ for each curve.
    }
    \label{fig:TempJump}
    \end{center}
\end{figure}
...
\begin{figure}
    ...
    \caption{
        ...
        The toxin $T_B$ causes the cell death of bacteria $A$ with rate constant $\mu = 0.2~\mathrm{s}^{-1}$.
        The red curve describes the growth of the bacteria $A$ ($N_A = L_A+G_A$) with model parameters $\lambda_A=10^{-2}~\mathrm{s}^{-1}$, $\alpha_A=3.0~\mathrm{s}^{-1}$.
        The blue curve describes the growth of the bacteria $B$ ($N_B = L_B+G_B$) with model parameters $\lambda_B=5\cdot 10^{-2}~\mathrm{s}^{-1}$, $\alpha_B=2.5~\mathrm{s}^{-1}$.
        The gray line denotes the dynamic of the resource pool scaled with its capacity $N_t=10^4$.
    }
...
\begin{align}
    ...
    \dot{G}_B &= \frac{\alpha_B}{1 + \mu_I I} R G_B + \lambda_B R L_B\\
    \dot{R} &= -\frac{\alpha_A}{N_t} R G_A-\frac{\alpha_B}{(1 + \mu_I I) N_t} R G_B\\
    \dot{I} &= \kappa G_A
\end{align}

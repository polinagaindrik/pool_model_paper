...
Stronger and quicker changes will lead to a higher rate $\gamma$, but the effect will relax to zero after some time.
After the extreme conditions are overcome the bacteria returns to a normal exponential growth with the transition rate $\xi$.
%
%
\paragraph{Maxwell type of stress - strain relation}
One of the reasons that bacteria enter the dormant state can be a shift in temperature conditions, e.g., to low temperatures unfavorable for bacteria growth~\citep{oliver_viable_1995}.
...
Note that here the direction of the temperature change is important, an increase from $T=2^\circ C$ to $T=12^\circ C$ have a different effect than a decrease from $T=14^\circ C$ to $T=4^\circ C$.
$\zeta$ is a scaling factor, $\sigma$ is a kind of stiffness telling how strongly the bacteria react to temperature stress, and $\delta^{-1}$ is the relaxation time for the temperature disturbance.
...
\end{equation}
One possible choice for $f$ reads:
\begin{align}
f(x)&=\begin{cases}\alpha x& x\ge 0\\ -(1-\alpha) x& x < 0,\end{cases}
\label{eq:f_stress}
\end{align}
$\alpha\in [0,1]$. For $\alpha=0.5$ it follows $f(x)=\frac{1}{2}|x|$ that represents the case where the direction of the temperature change does not matter.
...
The reasonable assumption is the linear dependence of the growth and lag-phase rates:
\begin{align}
    \begin{split}
        \alpha(T)  &= \alpha_1 T\\
        \lambda(T) &= \lambda_1 T.
...
In case of increase of the temperature the transition into the dormant state is much less so the effect is not noticeable while in contrast the division of bacteria is stimulated by more preferable conditions.
%
% ================================================================================================

...
Another special case of inter-species competition is when the produced toxin does not cause the death of the bacteria but rather inhibit their growth.
...
To construct our \ac{abm}, we use the modeling framework \texttt{cellular\_raza}~\citep{Pleyer2025}.
 allows us to define our bacterial agents from a clean slate.
...
Furthermore, we let cells of species $A$ produce a freely diffusable inhibitor $I$ with production rate $b$ and diffusion constant $D_I$ which inhibits cells of species $B$ in their uptake of nutrients.
\begin{equation}
...
\begin{align}
    \begin{split}
        ...
        \dot{V_{c_B}} &= u \sigma [R]V_c\\
        \dot{V_{c_A}} &= \frac{u}{1+\nu_I [I]} \sigma [R]V_c\\
        % \dot{[R_e]} &= \sum\limits_c -\frac{u}{1+\nu_I [I]} \frac{V_c}{V_D} [R] \delta(x-x_c) + D_R\Delta [R]\\
        \dot{[R]} &= u \sum\limits_{c_A}\frac{V_{c_A}}{V_D} [R_e] \delta(x-x_{c_A})\\
            &-\frac{u}{1+\nu_I [I]} \sum\limits_{c_B}\frac{V_{c_B}}{V_D} [R_e] \delta(x-x_{c_B})\\
            &+D_R \Delta [R_e]\\
...
The dynamics are described by newtondian motions for particle damping.
\begin{equation}
...
The \ac{abm} model should be considered to be more fundamental compared to the \ac{ode} description in the sense that it incorporates more biological aspects and maps them closer to the actual behaviour of the bacteria.
...
\begin{figure}
    \centering
    \begin{tikzonimage}[width=0.5\columnwidth]

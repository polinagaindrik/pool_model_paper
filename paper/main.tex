...
% ------------------------------------------------------------------------------------------------
\subsection{Dormancy}
Bacteria which have lived under rather unfavorable conditions have developed different \change{surviving }{survival} mechanisms, one of which is adapting their metabolism and transitioning into \change{}{a} dormant state.
...
This survival strategy can appear in response to different external stresses, e.g., nutrient deficiency, temperature stress or extreme fluctuations of \change{environment}{environmental} parameters.
\change{And after}{Having survived these} "difficult times" bacteria can continue \change{cell division}{growing and dividing}~\citep{kell_viability_1998}.
\change{Multiple models}{There exists a variety of} which include transitions of bacteria to the dormant state were proposed by \citet{bar_modelling_2002} and \citet{jones_dormancy_2010}.
%
We now aim to investigate the case where parts of the population $G$ enter a dormant state under stress conditions.
Within the framework of our pool model\change{}{,} this means that we define a new metabolic state $S$ corresponding to the bacterial stress.
\begin{align}
    L &\stackrel{\lambda}{\longrightarrow} G\\
    G &\stackrel{\mu}{\longrightarrow} 2G\\
    G &\stackrel{\omega'}{\longrightarrow} D\\
...
\end{align}
The transition rate of the growing bacteria to the stress state $\gamma(t)$ depends on the change of the environment \cmt{does it really depend on the "change" of the environment? Or does it depend in general on environmental conditions?}.
Stronger and quicker changes will lead to a higher rate $\gamma$, but the effect will relax to zero after some time. \cmt{Why is this the case?}
After the extreme conditions are overcome the bacteria \change{returns to a}{return to} normal exponential growth with the transition rate $\xi$.
%
%
\paragraph{Maxwell type of stress - strain relation}
One of the reasons that bacteria enter the dormant state can be a shift in temperature conditions, e.g., to low temperatures\change{}{,} unfavorable for \change{bacteria}{bacterial} growth~\citep{oliver_viable_1995}.
To model \change{it}{this} we propose a kind of viscoelastic Maxwell type of stress-strain relation and write down the ordinary differential equation for $\gamma$ to be:
\begin{equation}
...
Note that here the direction of the temperature change is important, an increase from $T=2^\circ C$ to $T=12^\circ C$ \change{have}{has} a different effect than a decrease from $T=14^\circ C$ to $T=4^\circ C$.
$\zeta$ is a scaling factor, $\sigma$ \change{is a kind of stiffness telling}{resembles a stiffness which quantifies} how strongly the bacteria react to temperature stress.
\change{and}{Finally,} $\delta^{-1}$ is the relaxation time for the temperature disturbance.
...
\end{equation}
One possible choice \cmt{can we motivate this better? Why is this choice taken?} for $f$ reads:
\begin{align}
f(x)&=\begin{cases}\alpha x& x\ge 0\\ -(1-\alpha) x& x < 0,\end{cases}
\label{eq:f_stress}
\end{align}
$\alpha\in [0,1]$. For $\alpha=0.5$ it follows $f(x)=\frac{1}{2}|x|$ \change{that}{which} represents the case where the direction of the temperature change does not matter.
...
The reasonable assumption is \change{the}{a} linear dependence of the growth and lag-phase rates: \cmt{why is this a reasonable assumption? Can we provide a reference possibly?}
\begin{align}
    \begin{split}
        \alpha(T)  &= \alpha_1 T\\
        \lambda(T) &= \lambda_1 T.
...
\begin{figure}[t]
    ...
    \end{center}
\end{figure}
The effect of the temperature shift on the total bacterial count \change{can be}{is} shown in Figure~\ref{fig:TempJump}.
...
Due to the rapid temperature decrease, the general growth slows down \change{till the}{untill} dormant bacteria migrate back to the growth pool.
\change{In case of increase of}{Upon increasing} the temperature\change{}{,} the transition into the dormant state is \change{much less so}{less pronounced such that} the effect is not noticeable while in contrast \cmt{I do not understand why you write "while in contrast". Is it really in contrast or is it just "in addition"? Maybe I have not correctly understood this.} the division of bacteria is stimulated by more preferable conditions.
%
% ================================================================================================
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{@{}l *2{>{\centering\arraybackslash}X} l *2{>{\centering\arraybackslash}X}@{}}
        ...
        \toprule
        Production $G_A$            & $\mu_A$    & $0.002$~\unit{\per\minute}
        & Uptake                    & $u_A$         & $0.01$~\unit{\per\minute}\\
        Production $G_B$            & $\mu_B$    & $0.002$~\unit{\per\minute}
        & Uptake                    & $u_B$         & $0.01$~\unit{\per\minute}\\
        Production $I$              & $\kappa$      & $0.1$~\unit{\per\minute}
        & Production $I$            & $b$           & $0.1$~\unit{\per\minute}\\
        Inhibition                  & $K_I$         & $1.0\cdot 10^{-7}$~\unit{\per\micro\metre\cubed}

...
Nutrients or resources in the microenvironment with the concentration $R_e$ diffuse freely with the diffusion constant $D_R$.
Cells take up extracellular resources $R_e$ and convert them into biomass, which is measured in terms of cellular volume.
...
%
% Cells $c_A$ of species $A$ take up extracellular resources $R_e$ into the cell $R_{i,c_A}$ with rate $u$ in \unit{\per\second}.
...
% \begin{alignat}{3}
%     \dot{R_{i,c_A}} &=& u &R_e(x_{c_A}) - aR_{i,c_A}
%     \label{eq:spatial-intracellular-a}\\
%     \dot{R_{i,c_B}} &=& \frac{u}{1+\nu I} &R_e(x_{c_B}) - aR_{i,c_B}
%     \label{eq:spatial-intracellular-b}\\
%     \label{eq:spatial-extracellular-resources-pde}
%     \dot{R_e}(x) &=& -u &\sum\limits_{c_A}\frac{V_{c_A}}{V_D} R_e(x) \delta(x-x_{c_A})\nonumber\\
%         &&-\frac{u}{1+\nu I} &\sum\limits_{c_B}\frac{V_{c_B}}{V_D} R_e(x) \delta(x-x_{c_B})\nonumber\\
%         &&+D_R\Delta R_e(x)
% \end{alignat}
...
% Therefore, we obtain the relations $\frac{u}{1+\nu I}R_e(x_{c_B}) = aR_{i,c_B}$ and $u R_e(x_{c_A})=a R_{i,c_A}$ which we can use to eliminate the variable monitoring internal resource concentrations $R_{i,c}$.
...
\begin{alignat}{5}
    ...
    \dot{R}_e &=& -u \sum\limits_{c_A}V_{c_A} &R_e(x) \delta(x-x_{c_A})\\
        &&-\frac{u}{1+\nu I} \sum\limits_{c_B}V_{c_B} &R_e(x) \delta(x-x_{c_B}) &+& D_R \Delta R_e(x)\nonumber\\
...
The equations~\eqref{eq:spatial-full-system-simplified} related to nutrient uptake and growth and its inhibition are
\begin{alignat}{3}
    \dot{V}_{c_B} &=& u_B \sigma &RV_c\\
    \dot{V}_{c_A} &=& \frac{u_A}{1+\nu I} \sigma &R V_c\\
    \dot{R}_e &=& -u_A \sum\limits_{c_A}V_{c_A} &R_e(x) \delta(x-x_{c_A})\\
        &&-\frac{u_B}{1+\nu I} \sum\limits_{c_B}V_{c_B} &R_e(x) \delta(x-x_{c_B}) &+& D_R \Delta R_e(x)\nonumber\\
    \dot{I} &=&b\sum\limits_{c_A} V_{c_A}&\delta(x-x_c) &+& D_I\Delta I.
\end{alignat}

...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% ONLY FOR EDITING REMOVE IN THE END
\usepackage{ulem}
\newcommand{\change}[2]{
    \ifthenelse{\equal{#1}{}}
    {\textcolor{purple}{#2}}
    {\textcolor{gray}{[\sout{#1}]} \textcolor{purple}{#2}}
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\author[1]{Jonas Pleyer\orcidlink{0000-0003-0928-3951}}
...
\begin{abstract}
    ...
    Their dynamics on a population-based level often result from various intra- and interspecies interactions,
...
In particular, food matrices represent highly relevant microbial ecosystems, where these communities
play a key role in both food safety and spoilage as well as fermentation processes.
Biofilms are biological structures formed by complex and dynamic microbial communities adhered to
surfaces that develop different communication mechanisms (e.g. quorum sensing), and coordinate to
improve nutrient accessibility and provide protection against antimicrobial chemicals and physical
stress~\citep{Zhao2023}.
These structures are an important problem in food environments, and healthcare settings, being a
source of contamination and increased risk of infection.
...
In addition, the natural bacterial processes are a crucial part of our planet's ecosystem, being
responsible for tasks such as nutrient cycling, decomposition of organic matter and oxygen production.
...
referred to as lag time~\citep{rolfe_lag_2012, monod_growth_1949}.
Under this phase, bacteria prepare the enzymatic and metabolic machinery to initiate cell division
and multiplication.
According to Baranyi and Roberts~\citep{baranyi_dynamic_1994}, lag time can
be defined as a function of the concentration of a limiting component that cells should synthesise
...
bacterial growth dynamics~\citep{perez-rodriguez_predictive_2012, gaindrik_experimental_2025}.
...
model~\citep{baranyi_modeling_1993, baranyi_dynamic_1994} and McKellar~\cite{McKellar1997} two-compartment model~\cite{buchanan_when_1997} which distinguishes between growing and non-growing bacteria.
...
individually-based \acp{abm} and can be effective, especially when dealing with large population of
...
Bacterial growth undergoes different consecutive phases~\citep{buchanan_when_1997}, starting with the so called lag-phase, which is a specific case of quiescence, during which no increase in the bacterial count occurs.
...
It is a logical assumption of the speicific case of cell adaption if environmental conditions do not
change.
...
We choose a minimalistic approach to describe the dependence of growth on the nutrient availability.
...
Then $[\mu_0]=\unit{V/h}$, $[N_t]=\unit{1/V}$, and $[\mu]= \unit{1/h}$.
$N_t$ denotes the maximal size of the total bacterial population due to environmental limitations.

For simplicity, we make a transition to the constant $\mu:=\mu_0 N_t$ and the dimensionless resource pool $\tilde{R}(t) = R/ N_t$ ($0 \leqslant \tilde{R} \leqslant 1$).
...
\begin{figure}
    ...
    \caption{
        The time dependence of the bacterial count $N(t) = L(t) + G(t) $ is shown calculated using the model (\ref{eq:ode_3pool_resource2}) with parameters $\mu=2.5~\mathrm{h}^{-1}$, $\omega=0~\mathrm{h}^{-1}$, $N_t = 10^4$.
        ...
        Other curves represent the growing bacteria dying with the rate $\omega'=0.1~\mathrm{h}^{-1}$ and different lengths of lag phases: short $\lambda=10^{-2}~\mathrm{h}^{-1}$ (dark green), intermediate $\lambda=10^{-3}~\mathrm{h}^{-1}$ (light green) and long one $\lambda=10^{-4}~\mathrm{h}^{-1}$ (yellow).
        Note that the steady state for $N(t)$ and $D(t)$ is for this model $\lim_{t\to\infty} N(t)=0$ and $\lim_{t\to\infty} D(t)=N_t$, because of the depletion of the resource pool.
    }
...
The relevant question here is whether cells in the pool $D$ provide any significant impact on resource availability, e.g., by freeing previously occupied spaces or releasing previously consumed nutrients.
...
This approach differs to other concepts of lag time such as the one by Gompertz (visual lag time) or Baranyi (work to be done).
...
% ------------------------------------------------------------------------------------------------
\subsection{Dormancy}
Bacteria which have lived under rather unfavorable conditions have developed different survival mechanisms, one of which is adapting their metabolism and transitioning into a dormant state.
Dormancy in bacteria is a reversible state where cells continue living with low metabolic activity while the cell division stops~\citep{kaprelyants_dormancy_1993}.
This survival strategy can appear in response to different external stresses, e.g., nutrient deficiency, temperature stress or extreme fluctuations of microenvironment parameters.
Having survived these "difficult times" bacteria can continue growing and dividing~\citep{kell_viability_1998}.
There exists a variety of models which include transitions of bacteria to the dormant state were proposed by \citet{bar_modelling_2002} and \citet{jones_dormancy_2010}.
%
We now aim to investigate the case where parts of the population $G$ enter a dormant state under stress conditions.
Within the framework of our pool model, this means that we define a new metabolic state $S$ corresponding to the bacterial stress.
\begin{align}
    L &\stackrel{\lambda}{\longrightarrow} G\\
    G &\stackrel{\mu}{\longrightarrow} 2G\\
    G &\stackrel{\omega'}{\longrightarrow} D\\
...
\end{align}
The transition rate of the growing bacteria to the stress state $\gamma(t)$ depends on the change of the environment.
Stronger and quicker changes will lead to a higher rate $\gamma$, but the effect will relax to zero after some time~\citep{lennon_microbial_2011}.
...
One of the reasons that bacteria enter the dormant state can be a shift in temperature conditions, e.g., to low temperatures, unfavorable for bacterial growth~\citep{oliver_viable_1995}.
To model this, we propose a kind of viscoelastic Maxwell type of stress-strain relation and write down the ordinary differential equation for $\gamma$ to be:
\begin{equation}
    \dot{\gamma} = \sigma f\left(\zeta\dot{T}\right)-\delta \gamma.
\end{equation}
The dimensionless function $f$, with $f(0)=0$ and $f: \mathbb{R}\to\mathbb{R}_{\ge 0}$, evaluates the effect of a temperature change.
Note that here the direction of the temperature change is important, an increase from $T=2^\circ C$ to $T=12^\circ C$ has a different effect than a decrease from $T=14^\circ C$ to $T=4^\circ C$.
$\zeta$ is a scaling factor, $\sigma$ resembles a stiffness which quantifies how strongly the bacteria react to temperature stress.
Finally, $\delta^{-1}$ is the relaxation time for the temperature disturbance.
Integration of this equation yields (with $\gamma(0)=0$):
\begin{equation}
    \gamma(t) = \sigma\int_0^t f\left(\zeta\dot{T}(t^{\prime})\right)e^{-\delta (t-t^{\prime})}dt^{\prime}.
\end{equation}
The simplest model choice to account for directional temperature changes is to use a non-symmetrical linear function:
\begin{align}
...
\begin{equation}
    \gamma(t) = \sigma\alpha\zeta\left [T(t)-T(0)e^{-\delta t}-\delta
        \int\limits_0^t T(t^{\prime})e^{-\delta (t-t^{\prime})}dt^{\prime}\right ],
\end{equation}
for $\dot{T}\ge 0, \,\forall t$ and
\begin{equation}
    \gamma(t) = \sigma(1-\alpha)\zeta\left [T(0)e^{-\delta t}-T(t)+\delta
        \int\limits_0^t T(t^{\prime})e^{-\delta (t-t^{\prime})}dt^{\prime}\right ].
\end{equation}
...
\begin{eqnarray}
    \gamma(t) &=& \sigma\sum_{i=1}^n f\left(\zeta\Delta T_i \right)\theta(t-t_i)e^{-\delta(t-t_i)}.
...
\begin{align}
    \begin{split}
        \dot{L} &= - \lambda L\\
        \dot{G} &= \lambda L + \mu G\left(1-\frac{L+G+D}{N_t}\right)-\gamma(t) G + \xi S - \omega' G\\
        \dot{D} &= \omega' G\\
...
For simplicity, we assume a linear dependence of the growth and lag-phase rates with respect to the temperature: 
\begin{align}
    \begin{split}
        \mu(T)  &= \mu_1 T\\
        \lambda(T) &= \lambda_1 T.
...
\begin{figure}[t]
    ...
    \caption{
        (A) The time dependence of the total bacterial count $N$ for the model including the transition to dormant state due to temperature stress~(\ref{eq:ode_tempshift_backlag}-\ref{eq:rates_temp_linear}).
        ...
        The curves represent no temperature shift (green), two temperature jumps $\Delta T = -5^\circ C, -5^\circ C$ at times $t=1~\mathrm{h}$ and $t=5~\mathrm{h}$ (orange)
        and the series of different temperature shifts of value $10^\circ C$, $-10^\circ C$ and $-5^\circ C$ at the time points $1~\mathrm{h}, 3~\mathrm{h}$ and $7~\mathrm{h}$, respectively (rose).
        The parameters associated with dormancy are $\zeta=1~\mathrm{h}/^\circ C $ and $\delta=5~\mathrm{h}^{-1}$, $\sigma=2~\mathrm{h}^{-2}$, $\xi=10^{-3}~\mathrm{h}^{-1}$.
        The growth and lag-phase rate linearly depend on the temperature $T$ and defined as $\mu(T)=0.2 T~\mathrm{h}^{-1}$, $\lambda(T)=10^{-2} T~\mathrm{h}^{-1}$.
        The other model parameters are $\omega'=0.1~\mathrm{h}^{-1}$, $N_t=10^4$.
    }
    \label{fig:TempJump}
    \end{center}
\end{figure}
The effect of the temperature shift on the total bacterial count is shown in Figure~\ref{fig:TempJump}.
There we can see the asymmetric behavior of the rate $\gamma$ depending on the direction of the temperature change (at $t=1~\mathrm{h}$).
Due to the rapid temperature decrease, the general growth slows down till the dormant bacteria migrate back to the growth pool.
Upon increasing the temperature, the transition into the dormant state is less pronounced ($\alpha=0.05$) such that the effect is not noticeable while in addition the division of bacteria is stimulated by more preferable conditions.
%
% ================================================================================================
...
The simplest pool model for bacterial species $A$ and $B$ encompassing lag-phases, growth-phases (no back-flow to the lag-pool) and a common resource pool is given by:
\begin{align}
    \begin{split}
        \dot{L}_A &= - \lambda_A R L_A\\
        \dot{L}_B &= - \lambda_B R L_B \\
        \dot{G}_A &= \lambda_A R L_A +\mu_A R G_A\\
        \dot{G}_B &= \lambda_B R L_B +\mu_B R G_B\\
        \dot{R} &=-\frac{\mu_A}{N_t} R G_A-\frac{\mu_B}{N_t} R G_B.
    \end{split}
...
\begin{align}
    ...
    \dot{G}_A &=  \lambda_A  L_A + \mu_A G_A - \frac{\lambda_A L_A + \mu_A G_A}{N_t} N\\
    \dot{G}_B &=  \lambda_B L_B + \mu_B G_B - \frac{\lambda_B L_B + \mu_B G_B}{N_t}N
\end{align}
...
Some examples of such mechanisms are the production of antimicrobial compounds, inhibitors (section~\ref{section:toxin-production}) or toxins~\citep{wloch-salamon_effect_2008, chao_structured_1981}.
...
\begin{align}
    \begin{split}
        \dot{L}_A &= - \lambda_A R L_A\\
        \dot{L}_B &= - \lambda_B R L_B \\
        \dot{G}_A &= \lambda_A R L_A + \mu_A R G_A - \frac{\omega}{N_t} T G_A\\
        \dot{G}_B &= \lambda_B R L_B + \mu_B R G_B\\
        \dot{R} &= -\frac{\mu_A}{N_t} R G_A-\frac{\mu_B}{N_t} R G_B\\
...
\begin{figure}
    ...
    \caption{
        The two species system competing for the common nutrient $R$ with production of the toxin $T_B$ (brown line) calculated using equations~(\ref{eq:model_2sp_toxin}).
        The growing bacteria $B$ produces toxin $T_B$ (A) constantly $\dot{T}_B^1 = \mathcal{F}^1 = \kappa G_B$ or (B) only during growth process $\dot{T}_B^2 = \mathcal{F}^2 = \kappa R G_B$ 
        with production rate $\kappa=0.2~\mathrm{h}^{-1}$.
        The toxin $T_B$ causes the cell death of bacteria $A$ with rate constant $\omega = 1~\mathrm{h}^{-1}$.
        The red curve describes the growth of the bacteria $A$ ($N_A = L_A+G_A$) with model parameters $\lambda_A=10^{-2}~\mathrm{h}^{-1}$, $\mu_A=3.0~\mathrm{h}^{-1}$.
        The blue curve describes the growth of the bacteria $B$ ($N_B = L_B+G_B$) with model parameters $\lambda_B=5\cdot 10^{-2}~\mathrm{h}^{-1}$, $\mu_B=2.5~\mathrm{h}^{-1}$.
        The gray line denotes the dynamic of the resource pool scaled with its capacity $N_t=10^4$.
    }
...
This leads to a toxin accumulation in the steady state $T_B^{2s}=\lim_{t\to\infty}T_B^2(t)=\kappa/\mu_B(G_B^s-N_{B0})$.
To see this we note $\dot{T}_B^2=\kappa/\mu_B(\dot{L_B}+\dot{G_B})$, which yields: $T_B^2(t)=\kappa/\mu_B(L_B(t)+G_B(t)-N_{B0})$ where we used $T_B^2(0)=G_B(0)=0$, $L_B(0)=N_{B0}$.
%
% ................................................................................................
...
\begin{align}\begin{split}
    \dot{G}_A &=\tilde{\mu}_A\frac{G_A}{G_B}\left(1 - G_A-G_B\right)\\
    \dot{G}_B &= \tilde{\mu}_B\frac{G_B}{G_A}\left(1 - G_A-G_B\right).
\end{split}\end{align}
...
Setting $\mathcal{B}_A(P_B,R)=\mu_A P_BR$, $\mathcal{B}_B(P_A,R)=\mu_B P_A R$, $\mathcal{F}_A(R,G_A)=\kappa_A RG_A$, $\mathcal{F}_B(R,G_B)=\kappa_B RG_B$, rescaling $G_A$, $G_B$, and $P_B$ with $N_t$, $P_A$ with $\kappa_A/\mu_A$, time with $\mu_AN_t^2$, defining $\psi=\kappa_A\mu_B/(\mu_A^2N_t)$, and $\phi=\kappa_B/(\mu_A N_t)$ results in
\begin{align}
    \begin{split}
...
terms of cellular processes, thus bridging the gap from a collective (bulk) model to an individual-based (single-cell) description. Our goal is to provide an \ac{abm} which can be used to derive the given \ac{ode} model as a coarse-grained approximation. In this way, we can show that the bulk-based \ac{ode} model is directly related to the mechanistic
single-cell behavior of the bacteria. To this end, we consider a system consisting of two bacterial species that can be described by the two internal states: lag and growth phase.
...
amount.  dThe \acp{ode} system of the bacterial population can be written as:
%
%
\begin{align}
    \begin{split}
        ...
        \dot{G}_A &= \lambda_A R L_A + \mu_A R G_A\\
        \dot{L}_B &= -\lambda_B R L_B\\
        \dot{G}_B &= \lambda_B R L_B + \frac{\mu_B}{1 + K_I I} R G_B.
    \end{split}
...
\begin{align}
    \begin{split}
        \dot{R} &= -\frac{\mu_A}{N_t} R G_A-\frac{\mu_B}{(1 + K_I I) N_t} R G_B \\
        \dot{I} &= \kappa G_A.
    \end{split}
...
We denote quantities of cellular species $A,B$ with the subscript $c_{A/B}$, respectively, and indicate shared cellular behavior by the subscript $c$.
% The equations stated below are motivated and obtained in greater detail in the supplementary material (section~\ref{ssec:supplement2}).
Nutrients or resources in the microenvironment with the concentration $R$ diffuse freely with the diffusion constant $D_R$.
...
%
% Cells $c_A$ of species $A$ take up extracellular resources $R$ into the cell $R_{i,c_A}$ with rate $u$ in \unit{\per\second}.
...
% \begin{alignat}{3}
%     \dot{R_{i,c_A}} &=& u &R(x_{c_A}) - aR_{i,c_A}
%     \label{eq:spatial-intracellular-a}\\
%     \dot{R_{i,c_B}} &=& \frac{u}{1+\nu I} &R(x_{c_B}) - aR_{i,c_B}
%     \label{eq:spatial-intracellular-b}\\
%     \label{eq:spatial-extracellular-resources-pde}
%     \dot{R}(x) &=& -u &\sum\limits_{c_A}\frac{V_{c_A}}{V_D} R(x) \delta(x-x_{c_A})\nonumber\\
%         &&-\frac{u}{1+\nu I} &\sum\limits_{c_B}\frac{V_{c_B}}{V_D} R(x) \delta(x-x_{c_B})\nonumber\\
%         &&+D_R\Delta R(x)
% \end{alignat}
...
% Note that $R=R(x)$ explicitly depends on the spatial coordinate but we choose to omit its notation.
...
% In addition, we want to modify the cellular volume by growing it increasingly with rate $u$.
...
% With a rate $a$ and conversion $\sigma$, the cell takes up and converts the resource molecules into volume.
...
% Therefore, we obtain the relations $\frac{u}{1+\nu I}R(x_{c_B}) = aR_{i,c_B}$ and $u R(x_{c_A})=a R_{i,c_A}$ which we can use to eliminate the variable monitoring internal resource concentrations $R_{i,c}$.
...
\begin{alignat}{5}
    \dot{V}_{c_B} &=& u \sigma &RV_c
    \label{eq:spatial-full-system-simplified-begin}\\
    \dot{V}_{c_A} &=& \frac{u}{1+\nu I} \sigma &R V_c\\
    \dot{R}_e &=& -u \sum\limits_{c_A}V_{c_A} &R(x) \delta(x-x_{c_A})\\
        &&-\frac{u}{1+\nu I} \sum\limits_{c_B}V_{c_B} &R(x) \delta(x-x_{c_B}) &+& D_R \Delta R(x)\nonumber\\
    \dot{I} &=&b\sum\limits_{c_A} V_{c_A}&\delta(x-x_c) &+& D_I\Delta I
    \label{eq:spatial-full-system-simplified-end}
\end{alignat}
For the diffusion, we require Neumann (zero-flux) boundary conditions.
It has to be noted that the sums $\sum_{c_{A/B}}$ range over all individual cells of the respective species at any given time point.
This means explicitly, that the number of summands varies in time and is part of the overall dynamics.
We indicate individual-based behavior with indices of types $X_c,X_{c_{A/B}}$ whereas macroscopic quantities related to a whole species are labelled with $X_{A/B}$.
...
We can formulate the resulting \ac{ode} in terms of volumetric quantities of the \ac{abm}.
...
\begin{alignat}{9}
    \dot{V}_{L,A} &= &&&&&-\lambda_A V_{L,A} &\bar{R}\\
    \dot{V}_{L,B} &= &&&&&-\lambda_B V_{L,B} &\bar{R}\\
    \dot{V}_A &= &u_A\sigma N_t\bar{R} &V_A &&&+ \lambda_A V_{L,A} &\bar{R}\\
    \dot{V}_B &= &&&\frac{u_B\sigma}{1+\nu I}N_t \bar{R} &V_B &+ \lambda_B V_{L,B} &\bar{R}\\
    \dot{\bar{R}} &= &-u_A\frac{V_A}{V_D} &\bar{R} &-\frac{u_B}{1+\nu I} \frac{V_B}{V_D} &\bar{R}\\
    \dot{I} &= b \frac{V_A}{V_D}
\end{alignat}
where we have already introduced the non-dimensional, rescaled nutrient concentration $\bar{R}$.
The previous \ac{ode} equations are
\begin{align}
    \dot{L}_A &= -\lambda_A R L_A\\
    \dot{L}_B &= -\lambda_B R L_B\\
    \dot{G}_A &= \mu_A R G_A + \lambda_A R L_A\\
    \dot{G}_B &= \frac{\mu_B}{1 + K_I I} R G_B + \lambda_B R L_B\\
    \dot{R} &= -\frac{\mu_A}{N_t} R G_A-\frac{\mu_B}{(1 + K_I I) N_t} R G_B\\
    \dot{I} &= \kappa G_A.
\end{align}
...
\begin{align}
    \begin{split}
        G_{A/B} &= \frac{1}{\sigma}\frac{V_{A/B}}{V_D}\\
        L_{A/B} &= \frac{1}{\sigma}\frac{V_{L,A/B}}{V_D}\\
...
\begin{figure}
    ...
    \end{tikzonimage}%
    \caption{
        Simulation snapshots at $\SI{100}{\minute},\SI{200}{\minute},\SI{300}{\minute}$ and $\SI{400}{\minute}$
        corresponding to the parameters given in Table~\ref{tab:spatial_limit_init}.
...
It means that ODE could still be suitable for aqueous environments, where homogeneity is more probable.
...
From studies surrounding the branching patterns of bacterial colonies~\citep{Kawasaki1997,Matsushita1998}, we know that the diffusivity of
nutrients is of the order of the spatial extension rate of the bacterial colony.
The chosen diffusion constants $D_I=D_R=\{5,20\}~\unit{\micro\metre\squared\per\second}$ mimick a behavior of a fast and slowly diffusing system respectively.
\begin{figure*}
    \centering
    ...
    \end{tikzonimage}
    \caption{
        (A-D) Comparison of the \ac{ode} and \ac{abm} model for different configurations.
        (A) Homogeneous, fast diffusion
        (B) Homogeneous, slow diffusion
        (C) Heterogeneous, fast diffusion
        (D) Random, fast diffusion
        The diffusion constants are chosen such that $D_R=D_I=D={5,20}~\unit{\micro\metre\squared\per\second}$.
...
By releasing these extracellular signalling molecules (autoinducers) it is possible for the bacteria to synchronize the gene expression of the whole group~\citep{ng_bacterial_2009}.
...
\begin{tabularx}{\textwidth}{l @{\hspace{0.5cm}} l}
    \textbf{Conceptualization}              & Christian Fleck, Polina Gaindrik, Jonas Pleyer, Fernando Pérez Rodríguez\\
    \textbf{Analysis - \acs{ode} Models}    & Polina Gaindrik, Christian Fleck\\
    \textbf{Analysis - Spatial effects}     & Jonas Pleyer, Christian Fleck\\
    \textbf{Writing}                        & Christian Fleck, Polina Gaindrik, Jonas Pleyer, Fernando Pérez Rodríguez\\
...
% \renewcommand{\align}{\@ifstar{\oldalign*}{\stepcounter{suppequation}\oldalign}}
% \renewcommand{\alignat}{\@ifstar{\oldalignat*}{\stepcounter{suppequation}\oldalignat}}
...
\begin{align}
    \dot \mbx  &= \mbF(\mbx, \mbm, \mbp, \mbu) = \begin{pmatrix}
        ...
        \omega              & \omega'            & -\beta
    \end{pmatrix} \mbx\\
    \dot \mbm &= \mbH (\mbx, \mbm, \mbp, \mbu) = -\frac{\mu}{N_t} R G.
\end{align}
...
\begin{equation}
    \dot \mbx = \begin{pmatrix}
        -(\omega + \lambda) & 0       & 0 \\
         ...
         \omega &  \omega'& - \beta
    \end{pmatrix} 
    \mbx + \frac{\mu}{N_t} x^2 \sum_i^3 x^i \begin{pmatrix} 0 \\ 1 \\ 0  \end{pmatrix}.
...
We recall equations \eqref{eq:spatial-full-system-simplified-begin} to \eqref{eq:spatial-full-system-simplified-end} related to nutrient uptake, growth and inhibition
\begin{alignat}{3}
    \dot{V}_{c_B} &=& u_B \sigma &RV_c\\
    \dot{V}_{c_A} &=& \frac{u_A}{1+\nu I} \sigma &R V_c\\
    \dot{R}_e &=& -u_A \sum\limits_{c_A}V_{c_A} &R(x) \delta(x-x_{c_A})\\
        &&-\frac{u_B}{1+\nu I} \sum\limits_{c_B}V_{c_B} &R(x) \delta(x-x_{c_B}) &+& D_R \Delta R(x)\nonumber\\
    \dot{I} &=&b\sum\limits_{c_A} V_{c_A}&\delta(x-x_c) &+& D_I\Delta I.
\end{alignat}
...
\begin{alignat}{8}
    ...
    &= \int\limits_D\bigg[&-&u_A \sum\limits_{c_A}V_{c_A} R \delta(x-x_{c_A})
        &-&\frac{u_B}{1+\nu I} \sum\limits_{c_B}V_{c_B} R \delta(x-x_{c_B})
        +D_R \Delta R\bigg]\text{dVol}\\
    &= &-&u_A\sum\limits_{c_A}V_{c_A}\int\limits_D R \delta(x-x_{c_A}) \text{dVol}
        &-&\frac{u_B}{1+\nu I} \sum\limits_{c_B}V_{c_B} \int\limits_D R \delta(x-x_{c_B})\text{dVol}
        +D_R \int\limits_{\partial D} \nabla R\text{dVol}\\
    &= &-&u_A\sum\limits_{c_A} V_{c_A} R_{x=x_{c_A}}
        &-& \frac{u_B}{1+\nu I} \sum\limits_{c_B} V_{c_B} R_{x=x_{c_B}}
\end{alignat}
...
We now evaluate the resource concentration $R$ at the position of the individual cells and sum over all contributions.
By defining the spatially averaged (and thus homogeneous) resource concentration $\hat{R}=V_D^{-1}\int_D R \text{dVol}$, we can rewrite the previous eqations to
\begin{alignat}{8}
    \frac{\text{d}}{\text{d}t} \frac{1}{V_D}\int\limits_D R\text{dVol}
        &= &-&u_A\sum\limits_{c_A} \frac{V_{c_A}}{V_D} R_{x=x_{c_A}}
        &-& \frac{u_B}{1+\nu I} \sum\limits_{c_B} \frac{V_{c_B}}{V_D} R_{x=x_{c_B}}\\
        &= &-&u_A\sum\limits_{c_A} \frac{V_{c_A}}{V_D} \hat{R}
        &-& \frac{u_B}{1+\nu I} \sum\limits_{c_B} \frac{V_{c_B}}{V_D} \hat{R}\\
    \frac{\text{d}}{\text{d}t}\hat{R}
        &= &-&u_A\frac{V_A}{V_D} \hat{R} &-&\frac{u_B}{1+\nu I} \frac{V_B}{V_D} \hat{R}.
\end{alignat}
Using the spatially averaged resource concentration $\hat{R}$, we can drop the point of evaluation, i.e. $\hat{R}_{x=x_c}=\hat{R}$.
...
% This means that every contribution does not depend on the position at which it was evaluated, i.e., $R_{x=x_c}=R$.
Finally, we can perform the sum to gain the total volume of all cells $V_{A/B}=\sum_{c_{A/B}} V_{c_{A/B}}$.
In order to achieve the same normalization as used in the \ac{ode} version, we also divide by the total resource capacity $N_t$ and denote the resulting quantity as $\bar{R}=\hat{R}/N_t$.
\begin{equation}
    \frac{\text{d}}{\text{d}t}\bar{R} = -u_A\frac{V_A}{V_D} \bar{R} -\frac{u_B}{1+\nu I} \frac{V_B}{V_D} \bar{R}
\end{equation}
Following the same principle as before, we can integrate the inhibitor equation.
\begin{alignat}{7}
    \int\limits_D\dot{I} &= \int\limits_D \bigg[&&\sum\limits_{c_A} b V_{c_A}&&\delta(x-x_{c_A}) &&+ D_I\Delta I\bigg]\text{dVol}\\
    % &= \int\limits_D&&\sum\limits_{c_A} b V_{c_A}&&\delta(x-x_{c_A}) \text{dVol} &&+ \int\limits_D D_I\Delta I\text{dVol}\\
    &= \int\limits_D&&\sum\limits_{c_A} b V_{c_A}&&\delta(x-x_{c_A}) \text{dVol} &&+ \int\limits_{\partial D} D_I\nabla I\mathbf{\text{dA}}\\
    &= &&\sum\limits_{c_A} b V_{c_A}\int\limits_D &&\delta(x-x_{c_A}) \text{dVol}\\
    &= b V_A
\end{alignat}
By again using the spatially averaged inhibitor concentration $\hat{I}=V_D^{-1}\int_D I\text{dVol}$, we arrive at
\begin{equation}
    \frac{\text{d}}{\text{d}t} \hat{I} = b \frac{V_A}{V_D}
\end{equation}
%
\paragraph{Lag-Phase}
...
This expression can be transfered to a collective rate of change by multiplying with the cells in lag-phase $L_{A/B}$.
\begin{align}
    P(\text{transition}) &= \Delta t \lambda_{A/B} \frac{R}{N_t}\\
    \frac{1}{\Delta t}P(\text{transition}) &= \lambda_{A/B} \frac{R}{N_t}\\
    \frac{L_{A/B}}{\Delta t}P(\text{transition}) &= \lambda_{A/B} L_{A/B}\frac{R}{N_t}\\
    \frac{\text{d}}{\text{d}t} L_{A/B} &= -\lambda_{A/B} L_{A/B}\frac{R}{N_t}
\end{align}
In the last step, we take the expectation value and equate it with the negative of its derivative $\text{d}/\text{d}t L_{A/B}$.
When switching to a fully homogeneous system, we exchange $R$ with its spatially averaged counterpart $\hat{R}$.
Taking into account the normalization via $\bar{R}=\hat{R}/N_t$, we can write
\begin{equation}
    \frac{\text{d}}{\text{d}t}L_{A/B} = - \lambda_{A/B}L_{A/B}\bar{R}.
\end{equation}
%
%
\paragraph{Growth}
The equations describing the overall growth of the bacteria is governed by two distinct effects:
Bacteria are leaving the lag phase and starting to grow, thus adding them to this pool and bacteria already in the growth pool consume nutrients, thus growing and dividing.
For the equation describing growth of the cell volume due to nutrient uptake, we simply add up cells for the respective species to obtain their total volume.
\begin{alignat}{9}
    \dot{V}_{A,R} &= \sum\limits_{c_A}\dot{V}_{c_A} &=& &u_A\sigma\sum\limits_{c_A} &R V_{c_A} &=& &u_A\sigma& R V_A\\
    \dot{V}_{B,R} &= \sum\limits_{c_B}\dot{V}_{c_B} &=& &\frac{u_B\sigma}{1+\nu I} \sum\limits_{c_B} & R V_{c_B} &=& &\frac{u_B\sigma}{1+\nu I} &R V_B
\end{alignat}
We can see that this equation can be easily rescaled with appropriate constants such that we can use the same notion defined in section~\ref{sec:three-pool-model}
\begin{alignat}{3}
    \dot{G}_{A,R} &=& u_A\sigma R G_A\\
    \dot{G}_{B,R} &=& \frac{u_B\sigma}{1+\nu I} R G_B.
\end{alignat}
We combine this with the result obtained for the lag phase to obtain the complete dynamics
\begin{alignat}{5}
    \dot{G}_A &=& u_A\sigma N_t \bar{R} G_A &+ \lambda_A L_A &\bar{R}\\
    \dot{G}_B &=& \frac{u_B\sigma}{1+\nu I}N_t \bar{R} G_B &+ \lambda_B L_B &\bar{R}
\end{alignat}
where we have again introduced the rescaled and spatially averaged resource concenration $\bar{R}$.
These terms only apply to bacteria which are in a growing state and not in the lag phase.
We can thus summarize the equations obtained
\begin{alignat}{9}
    \dot{L}_A &= &&&&&-\lambda_A L_A &\bar{R}\\
    \dot{L}_B &= &&&&&-\lambda_B L_B &\bar{R}\\
    \dot{G}_A &= &u_A\sigma N_t\bar{R} &G_A &&&+ \lambda_A L_A &\bar{R}\\
    \dot{G}_B &= &&&\frac{u_B\sigma}{1+\nu I}N_t \bar{R} &G_B &+ \lambda_B L_B &\bar{R}\\
    \dot{\bar{R}} &= &-u_A\frac{V_A}{V_D} &\bar{R} &-\frac{u_B}{1+\nu I} \frac{V_B}{V_D} &\bar{R}\\
    \dot{I} &= b \frac{V_A}{V_D}
\end{alignat}
Comparing these equations with the \ac{ode} version defined by \eqref{eq:spatial_limit_F} and \eqref{eq:spatial_limit_H}, we can identify
\begin{align}
    \mu_{A/B} &= u_{A/B}\sigma N_t\\
    \frac{\mu_{A/B}}{N_t}G_{A/B} &= u_{A/B}\frac{V_{A/B}}{V_D}\\
    \kappa G_A &= b \frac{V_A}{V_D}
\end{align}
...
\begin{align}
    ...
    \kappa &= \sigma b.
\end{align}
Checking the units of these quantities, we have $[\sigma^{-1}]=[R]=1/V$ which matches the definition of section~\ref{sec:three-pool-model}.
Furthermore, we have $[b]=[R/t]=1/V/T$ and thus $[\kappa]=[\sigma b]=[1/t]$ which also matches equation~\eqref{eq:spatial_limit_H}.
Expressing the obtained equations again in terms of volumes $V_{A/B}$, we obtain
\begin{alignat}{9}
    \dot{V}_{L,A} &= &&&&&-\lambda_A V_{L,A} &\bar{R}\\
    \dot{V}_{L,B} &= &&&&&-\lambda_B V_{L,B} &\bar{R}\\
    \dot{V}_A &= &u_A\sigma N_t\bar{R} &V_A &&&+ \lambda_A V_{L,A} &\bar{R}\\
    ...
    \dot{I} &= b \frac{V_A}{V_D}
\end{alignat}
where we introduced the quantities $V_{L,A/B}$ which correspond to the volume encapsulated by the lag state of species $A/B$.
These equations can now be converted directly to the original \ac{ode} via
\begin{align}
    G_{A/B} &= \frac{1}{\sigma}\frac{V_{A/B}}{V_D}\\
    L_{A/B} &= \frac{1}{\sigma}\frac{V_{L,A/B}}{V_D}\\
    \frac{\mu_{A/B}}{N_t} &= u_{A/B}\sigma\\
    ...
    K_I &= \nu
\end{align}
where we have related \ac{ode}-specific parameters on the left side with \ac{abm}-specific ones on the right side.
%
%
% ................................................................................................
\vfill
...
\begin{table}[H]
    ...
    \begin{tabularx}{0.9\textwidth}{@{}l *1{>{\centering\arraybackslash}X} r l @{\hspace{1cm}} l *1{>{\centering\arraybackslash}X} r l @{}}
        \multicolumn{4}{c}{\textbf{\acs{ode}}} & \multicolumn{4}{c}{\textbf{\acs{abm}}}\\
        \toprule
        \multicolumn{8}{c}{\textbf{Cellular Reactions}}\\
        \toprule
        Production $G_A$            & $\mu_A$       & $6.25\cdot10^{-5}$  & \unit{\per\second}
        & Uptake                    & $u_A$         & $0.0025$  & \unit{\per\second}\\
        Production $G_B$            & $\mu_B$       & $6.25\cdot10^{-5}$  & \unit{\per\second}
        ...
        & Inhibition                & $\nu$         & $0.025$   & \unit{\micro\metre\cubed\per\mol}\\
        &&
        ...
        &&&& Potential               & $\epsilon$    & $0.03125$ & \unit{\pico\gram\micro\metre\per\second\squared}\\
        \toprule
...
\begin{figure}[H]
    ...
    \begin{subfigure}[c]{0.5\textwidth}
        \includegraphics[width=\textwidth]{Figures-abm-homogenous-images-snapshot_00024000.pdf}
        \subcaption{Homogeneous initial cell distribution, $D_R=D_I=20.0$~\unit{\micro\metre\squared\per\second}}
    \end{subfigure}%
    \begin{subfigure}[c]{0.5\textwidth}
        \includegraphics[width=\textwidth]{Figures-abm-homogenous-low-diffusion-images-snapshot_00024000.pdf}
        \subcaption{Homogeneous initial cell distribution, $D_R=D_I=2.0$~\unit{\micro\metre\squared\per\second}}
    \end{subfigure}\\
    \begin{subfigure}[c]{0.5\textwidth}
        \includegraphics[width=\textwidth]{Figures-abm-inhomogenous-images-snapshot_00024000.pdf}%
        \subcaption{Heterogeneous initial cell distribution, $D_R=D_I=20.0$~\unit{\micro\metre\squared\per\second}}
    \end{subfigure}%
    \begin{subfigure}[c]{0.5\textwidth}
        \includegraphics[width=\textwidth]{Figures-abm-random-images-snapshot_00024000.pdf}%
        \subcaption{Random initial cell distribution, $D_R=D_I=20.0$~\unit{\micro\metre\squared\per\second}}
    \end{subfigure}
    \caption{
...
\vfill
\pagebreak
\section{Remarks on comparing ABM and ODE results}
In order to compare the quantities defined by the \ac{abm} and the \ac{ode}, we can use the
conversion relations \eqref{eq:ode-abm-mappings}.
This allows us to understand the quantities $G_{A/B},L_{A,B},R$ in terms of the cellular quantities
defined by the \ac{abm}.
We have introduced the growth pools $G_{A/B}$ in such a way that they can be directly compared with
the resource pool $R$ which is immediately clear by looking at equations
\eqref{eq:ode_3pool_resource} to \eqref{eq:ode_3pool_resource2}.
Therefore we can not simply interpret $G_A$ as the cell density defined by the cell number divided
by the total volume of the domain.
However, equations \eqref{eq:ode-abm-mappings} allow us to investigate this matter further.

We start by expressing the total volume of species $A$ in terms of the total amount of cells.
To this end, we introduce the average cell volume $\bar{V}_c$ which can be assumed to be constant
for large enough populations.
We thus obtain
\begin{equation}
    V_A = N_A \bar{V}_c
\end{equation}
which we plug into equation~\ref{eq:ode-abm-mappings} and obtain
\begin{equation}
    G_A = \frac{1}{\sigma} N_A \frac{\bar{V}_c}{V_D}.
\end{equation}
We rearrange this equation and additionally assume that $G_A$ is proportional to the cell number
$N_A$ via
\begin{equation}
    G_A = a N_A
\end{equation}
and thus obtain
\begin{equation}
    a = \frac{1}{\sigma}\frac{\bar{V}_c}{V_D}.
\end{equation}
We consider the case in which we want to consume nutrients to add a single bacterium
($\Delta N_A=1$) of average volume $\bar{V}_c$ to $G_A$.
For this scenario, we calculate the difference in resources $\Delta R$ and $\Delta G_A$ to be
\begin{equation}
    - \Delta R = \Delta G_A = \frac{1}{\sigma}\frac{\bar{V}_c}{V_D}.
\end{equation}

We can apply these observations to the numerical results oft the \ac{abm} and \ac{ode} and thus
estimate quantities such as the cell number $N_{A/B}$ over the course of the simulation via
different methods.
First and foremost, the \ac{abm} tracks the total amount of bacteria and thus we can simply read out
this quantity.
Secondly, we can estimate this quantity by via the total volume $V_{A/B}$ and the average volume
$\bar{V}_c$ via
\begin{equation}
    N_A = \frac{V_A}{\bar{V}_c}.
\end{equation}
And lastly, we can use the results $G_A$ of the \ac{ode} and calculate $N_A$ via
\begin{equation}
    N_A = \sigma G_A \frac{V_D}{\bar{V}_c}.
\end{equation}
Figure~\ref{fig:spatial-growth-curve-cellcount} shows the numerical values for these approaches.
When comparing this plot with the volumetric-based one (Figure~\ref{fig:spatial-growth-curve}), we
see that the cell number is subject to fluctuations due to the low overall cellcount.
These fluctuations will be less noticable in larger systems containing significantly more bacterial
agents but show here.
In contrast, the volumetric plot presents a smooth curve, since individual jumps in cellcount do not
affect the overall amount of volume.
Therefore, the volumetric comparison was chosen since these curves are expected to be better
representative of a larger system.

\begin{figure}[H]
    \centering
    \begin{tikzonimage}[width=0.5\textwidth]
        {Figures-abm-homogenous-abm_ode_comparison-cellcount.pdf}
        \node at (0, 1)[anchor=north west]{\textbf{A}};
    \end{tikzonimage}%
    \begin{tikzonimage}[width=0.5\textwidth]
        {Figures-abm-homogenous-low-diffusion-abm_ode_comparison-cellcount.pdf}
        \node at (0, 1)[anchor=north west]{\textbf{B}};
    \end{tikzonimage}\\
    \begin{tikzonimage}[width=0.5\textwidth]
        {Figures-abm-inhomogenous-abm_ode_comparison-cellcount.pdf}
        \node at (0, 1)[anchor=north west]{\textbf{C}};
    \end{tikzonimage}%
    \begin{tikzonimage}[width=0.5\textwidth]
        {Figures-abm-random-abm_ode_comparison-cellcount.pdf}
        \node at (0, 1)[anchor=north west]{\textbf{D}};
    \end{tikzonimage}
    \caption{
        (A-D) Comparison of the \ac{ode} and \ac{abm} model for different configurations.
        We plot the celcount $N_{A/B/\text{Total}}$ obtained via different methods.
        (A) Homogeneous, fast diffusion
        (B) Homogeneous, slow diffusion
        (C) Heterogeneous, fast diffusion
        (D) Random, fast diffusion
    ...
    \label{fig:spatial-growth-curve-cellcount}
\end{figure}

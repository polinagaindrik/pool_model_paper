...
We denote quantities of cellular species $A,B$ with the subscript $c_{A/B}$, respectively, and indicate shared cellular behavior by the subscript $c$. The equations stated below are motivated and obtained in greater detail in the supplementary material (section~\ref{ssec:supplement2}). Nutrients or resources in the microenvironment with the concentration $R_e$ diffuse freely with the diffusion constant $D_R$. Cells take up extracellular resources $R_e$ and convert them into biomass, which is measured in terms of cellular volume. Cells of type $A$ release a diffusible molecule into the microenvironment with concentration $I$ and diffusion constant $D_I$, which inhibits the growth of the cells of type $B$. 
...
%         &&-\frac{u}{1+\nu_I I} &\sum\limits_{c_B}\frac{V_{c_B}}{V_D} R_e(x) \delta(x-x_{c_B})\nonumber\\
%         &&+D_R\Delta R_e(x)
% \end{alignat}
...
% Therefore, we obtain the relations $\frac{u}{1+\nu_I I}R_e(x_{c_B}) = aR_{i,c_B}$ and $u R_e(x_{c_A})=a R_{i,c_A}$ which we can use to eliminate the variable monitoring internal resource concentrations $R_{i,c}$.
This results into the set of equations: 
...
\begin{alignat}{5}
    \dot{V}_{c_A} &=& u \sigma &R_e(x_A)V_{c_A}\label{eq:spatial-full-system-simplified-begin}\\
    \dot{V}_{c_B} &=& \frac{u}{1+\nu_I I(x_B)} \sigma &R_e(x_B) V_{c_B}\\
    \dot{R}_e(x) &=& -u \sum\limits_{c_A}\frac{V_{c_A}}{V_D} &R_e(x) \delta(x-x_{c_A})\\
        &&-\frac{u}{1+\nu_I I(x)} \sum\limits_{c_B}\frac{V_{c_B}}{V_D} &R_e(x) \delta(x-x_{c_B}) &+& D_R \Delta R_e(x)\nonumber\\
    \dot{I}(x) &=&b\sum\limits_{c_A} \frac{V_{c_A}}{V_D}&\delta(x-x_c) &+& D_I\Delta I(x)
    \label{eq:spatial-full-system-simplified-end}
\end{alignat}
We use the Dirac delta-distribution $\delta(x-x_c)$ to enforce interactions at the positions of the cells $x_{c}$. $\sigma$ is the conversion rate of nutrients into cellular volume and $u_c$ defines the rate of this process. For the diffusion, we require Neumann (zero-flux) boundary conditions. Note that we made the dependence of $R_e$ and $I$ on the spatial coordinate $x$ explizit for clarity reasons. 
...
As the coupled reactions from equations~\eqref{eq:spatial-full-system-simplified-begin}-\eqref{eq:spatial-full-system-simplified-end} change $V_c$, we also automatically adapt the cell radius accordingly.
...
The dimensionality of the problem does not find its way into the cellular reactions but only manifests itself in these mechanical considerations. {\bf [Verstehe nicht, was du damit meinst]}
The dynamics of the cells are governed by Newton's law of motion with damping or friction:
\begin{equation}
    m \ddot{\mathbf{x}} = -\eta\dot{\mathbf{x}} + \mathbf{F}.
\end{equation}
Cells are interacting via a point-like interaction potential. We can calculate the interaction strength via
\begin{alignat}{3}
...
\end{alignat}
{\bf [was sind $r_1$ und $r_2$? Welche Rolle spielt $q$?]}
We consider only a repulsive interaction potential between cells, leaving out any possible attractive forces~\citep{Garrett2008}.
...
We are thus able to derive the \ac{ode} pool model from its \ac{abm} version via a process of homogenization (see Appendix~\ref{ssec:supplement3}).
...
\vfill
\pagebreak
\section{Motivating the Cellular Reactions of the \ac{abm}}
\label{ssec:supplement2}
The equations governing the dynamics of the \ac{abm} cannot be directly derived from higher principles but one can motivate them via the following considerations.
Our model consists of individual cells with volume $V_c$ and extracellular nutrients $R_e(x)$ which are spatially distributed. We set up a balancing equation that aims to relate these concepts.
Assuming that any loss in extracellular nutrients $R_e$ is directly converted into cellular volume with a conversion factor $\sigma$ ($[\sigma]=V$), the balancing equation then reads:
\begin{equation}
    \text{const} = \sigma \int\limits_D R_e(x)\text{dVol} + \sum\limits_c V_c.
\end{equation}
We now assume that there are two species $A,B$ present and split the sum and since we are interested in the dynamics of the system, we also take the temporal derivative.
By adding a zero in the form of:
\begin{equation}
   0= u_c\sigma \sum\limits_c\int\limits_D\frac{V_c}{V_D}R_e(x)\delta(x-x_c)\text{dVol}-u_c\sigma \sum\limits_c\int\limits_D\frac{V_c}{V_D}R_e(x)\delta(x-x_c)\text{dVol},
\end{equation}
we can introduce an exchange term between extracellular nutrients and cellular volume.
In total we obtain
\begin{alignat}{3}
    0 &= \sigma\int\limits_D\dot{R}_e(x)\text{dVol} &+ \sigma\sum\limits_c u_c&\int\limits_D\frac{V_c}{V_D}R_e(x)\delta(x-x_c)\text{dVol}\nonumber\\
    &&- \sigma\sum\limits_c u_c&\int\limits_D\frac{V_c}{V_D}R_e(x)\delta(x-x_c)\text{dVol} + \sum\limits_c \dot{V}_c\\
    0 &= \sigma\int\limits_D\Big[\dot{R}_e(x) &+ \sum\limits_c u_c&\frac{V_c}{V_D}R_e(x)\delta(x-x_c)\Big]\text{dVol}\nonumber\\
    &&- \sum\limits_c\Big[\sigma u_c&V_cR_e(x_c) + \dot{V}_c\Big].
     \label{eq:supplement-abm-derivation-intermediate-4}
\end{alignat}
In order to satisfy this balancing equation, we can enforce equality of the dynamics for the subsystems given by \ref{eq:supplement-abm-derivation-intermediate-4}.
These can be viewed as the extracellular dynamics surrounding the nutrient resource $R_e$ and intracellular dynamics increasing the volume $V_c$.
We assume that the spatial distribution of $R_e$ is governed by a diffusion process with Neumann boundary conditions such that no additional nutrients are introduced and no nutrients lost.
In particular, any diffusion effects vanish in the integral
\begin{equation}
    \int\limits_D\Delta R_e(x)\text{dVol} = \int\limits_{\partial D}\nabla R_e(x)\text{d\textbf{A}} = 0
\end{equation}
...
So far, we have treated all cells equally but the system which we aim to describe consists of two distinct species $A/B$ which behave differently.
...
Another mechanism of this system is the inhibition of growth mediated by an external inhibitor $I$ which is produced by species $A$.
We assume that the release of inhibitor molecules can be neglected for the assumptions governing the initial balancing of the system.
In addition to the production of the inhibitor, we also introduce diffusion with constant $D_I$ which results in
\begin{equation}
...
\begin{alignat}{5}
    ...
    \dot{V}_{c_B} &= &\sigma \frac{u}{1+\nu_I I(x_{c_B})}&R_e(x_{c_B})V_c\\
    \dot{R}_e(x) &=
        &-\sigma u&\sum\limits_{c_B}\frac{V_{c_B}}{V_D}R_e(x)\delta(x-x_{c_B})\\
        &&-\sigma\frac{u}{1+\nu_II(x_{c_B})}&\sum\limits_{c_B} \frac{V_{c_A}}{V_D}R_e(x)\delta(x-x_{c_B}) &+& D_R\Delta R_e(x)\\
    \dot{I}(x) &= &b&\sum\limits_{c_A}\frac{V_{c_A}}{V_D}\delta(x-x_{c_A}) &+& D_I \Delta I(x).
\end{alignat}
%
%
...
\begin{alignat}{8}
    ...
    &= &-&u\sum\limits_{c_A} V_{c_A} R_{x=x_{c_A}}
        &-& \frac{u}{1+\nu_I I} \sum\limits_{c_B} V_{c_B} R_{x=x_{c_B}}\\
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{@{}l *2{>{\centering\arraybackslash}X} l *2{>{\centering\arraybackslash}X}@{}}
        ...
        \toprule
        Production $G_A$            & $\mu_A$       & $0.0005$~\unit{\per\second}
        & Uptake                    & $u_A$         & $0.0025$~\unit{\per\second}\\
        Production $G_B$            & $\mu_B$       & $0.0005$~\unit{\per\second}
        & Uptake                    & $u_B$         & $0.0025$~\unit{\per\second}\\
        Production $I$              & $\kappa$      & $0.025$~\unit{\per\second}
        & Production $I$            & $b$           & $0.025$~\unit{\per\second}\\
        Inhibition                  & $K_I$         & $0.25\cdot 10^{-7}$~\unit{\per\micro\metre\cubed}
        & Inhibition                & $\nu_I$       & $0.025$~\unit{\micro\metre\cubed\per\mol}\\
        &&
        & Conversion                & $\sigma$      & $0.025$~\unit{\micro\metre\cubed\per\mol}\\
        Total volume                & $N_t$& $0.5\cdot 10^5$~\unit{\micro\metre\cubed}
        &&&\\
        &&
        ...
        Volume of $L_B$             & $L_B$         & $127.2$~\unit{\micro\metre\cubed}
        & Cell Number               & $C_B$         & 18\\

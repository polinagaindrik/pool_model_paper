...
\author[2]{Fernando Pérez Rodríguez}
...
Effects on a single-cell level such as adhesion~\citep{htuson_bacteriasurface_2013}, migration~\citep{decoene_microscopic_2011} and chemotaxis can lead to complex
...
model~\citep{baranyi_modeling_1993, baranyi_dynamic_1994} and McKellar~\citep{McKellar1997} two-compartment model~\citep{buchanan_when_1997} which distinguishes between growing and non-growing bacteria.
...
For simplicity, we make a transition to the dimensionless constant $\mu:=\mu_0 N_t$ and dimensionless resource pool $\tilde{R}(t) = R/ N_t$ ($0 \leqslant \tilde{R} \leqslant 1$).
...
Upon increasing the temperature, the transition into the dormant state is less pronounced such that the effect is not noticeable while in addition the division of bacteria is stimulated by more preferable conditions.
%
% ================================================================================================
...
\begin{align}
    \begin{split}
        \dot{L}_A &= - \lambda_A R L_A\\
        \dot{L}_B &= - \lambda_B R L_B \\
        \dot{G}_A &= \lambda_A R L_A + \mu_A R G_A - \frac{\omega}{\kappa N_t} T G_A\\
        \dot{G}_B &= \lambda_B R L_B + \mu_B R G_B\\
        \dot{R} &= -\frac{\mu_A}{N_t} R G_A-\frac{\mu_B}{N_t} R G_B\\
...
\begin{figure}
    ...
    \caption{
        ...
        The toxin $T_B$ causes the cell death of bacteria $A$ with rate constant $\omega = 0.2~\mathrm{h}^{-1}$.
        The red curve describes the growth of the bacteria $A$ ($N_A = L_A+G_A$) with model parameters $\lambda_A=10^{-2}~\mathrm{h}^{-1}$, $\mu_A=3.0~\mathrm{h}^{-1}$.
        The blue curve describes the growth of the bacteria $B$ ($N_B = L_B+G_B$) with model parameters $\lambda_B=5\cdot 10^{-2}~\mathrm{h}^{-1}$, $\mu_B=2.5~\mathrm{h}^{-1}$.
        The gray line denotes the dynamic of the resource pool scaled with its capacity $N_t=10^4$.
    }
...
\end{split}\end{align}
We introduced the rescaled growth rates $\tilde{\mu}_A=\mu_A/(N_tK_B)$ and $\tilde{\mu}_B=\mu_B/(N_tK_A)$.
Note that $[\tilde{\mu}]=\mathrm{h}^{-1}$.
We rescale the time with $\tilde{\mu}_A$ and define the dimensionless parameter
\begin{equation}
...
The presented equations are motivated and obtained in greater detail in the supplementary material (section~\ref{sec:supplement-abm-derivation}).
Cells $c_A$ of species $A$ take up extracellular resources $R_e$ and immediately convert it into biomass in the form of cellular volume $V_c$
\begin{equation}
    \dot{V}_c = u_c\sigma R_{i,c} V_c
\end{equation}
where $\sigma$ resembles a conversion rate of nutrients in cellular volume and $u_c$ defines the rate of this process.
Its value is influenced by the abundance of an external inhibitor $I$ which is produced by species $A$ and inhibits species $B$.
For species $B$, the rate is reduced via $u_{c_B}=u/(1+\nu_I I)$ while species $A$ remains unaffected $u_{c_A}=u$.
The inhibitor also diffuses freely in the given domain, resulting in
\begin{equation}
    \dot{I} = b\frac{V_{c_A}}{V_D}\delta(x-x_{c_A}) + D_I \Delta I.
\end{equation}
We use the Dirac delta-distribution $\delta(x-x_c)$ to enforce interactions at the positions of the cells $x_{c}$.
...
\begin{alignat}{5}
    ...
    \dot{R}_e &=& -u \sum\limits_{c_A}\frac{V_{c_A}}{V_D} &R_e(x) \delta(x-x_{c_A})\\
        &&-\frac{u}{1+\nu_I I} \sum\limits_{c_B}\frac{V_{c_B}}{V_D} &R_e(x) \delta(x-x_{c_B}) &+& D_R \Delta R_e(x)\nonumber\\
    \dot{I} &=&b\sum\limits_{c_A} \frac{V_{c_A}}{V_D}&\delta(x-x_c) &+& D_I\Delta I
...
The equations governing the dynamics of the \ac{abm} can not be directly derived from higher principles but can still aim to motivate them as best as possible.
Our model consists of individual cells with volume $V_c$ and extracellular nutrients $R_e(x)$ which are spatially distributed.
We set up a balancing equation that aims to relate these concepts.
To this end, we assume that any loss in extracellular nutrients is directly converted into cellular volume.
...
\begin{alignat}{3}
    ...
    &&- \sigma\sum\limits_c u_c&\int\limits_D\frac{V_c}{V_D}R_e(x)\delta(x-x_c)\text{dVol} + \sum\limits_c \dot{V}_c\\
     &= \sigma\int\limits_D\Big[\dot{R}_e(x) &+ \sum\limits_c u_c&\frac{V_c}{V_D}R_e(x)\delta(x-x_c)\Big]\text{dVol}
...
\end{alignat}
In order to satisfy this balancing equation, we can enforce equality of the dynamics for the subsystems given by \ref{eq:supplement-abm-derivation-intermediate-3} and \ref{eq:supplement-abm-derivation-intermediate-4}.
...
\begin{equation}
    \dot{I}(x) = b\sum\limits_{c_A}\frac{V_{c_A}}{V_D}\delta(x-x_{c_A}).
\end{equation}

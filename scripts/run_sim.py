...
def generate_cells(n_cells_1, n_cells_2, domain, randomness=0.0, pad=0.15, seed=0):
    ...
    positions_lattice = calculate_lattice_points(
        0.0, domain.size, n_cells_1 + n_cells_2
    )
    positions = positions_random * r + (1 - r) * positions_lattice
    ind1, ind2 = calculate_index_distributions(
        n_cells_1, n_cells_2, len(positions), homogenous=False
    )
    ...
    for i in range(n_cells_1 + n_cells_2):
        ...
        if i < n_cells_1:
            ...
            cell.cellular_reactions.species = crp.Species.S2
